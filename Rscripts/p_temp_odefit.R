...
library(simecol)
# Load the dplyr package for improved data manipulation functions
library(dplyr)
# Load the ggplot2 package for improved plotting functions
library(ggplot2)
...
# If you would like to fit the model using a different replicate, simply change the value of unique_ID
pdata <- filter(pdata, unique_ID == 1)
...
parms <- c("r", "K", "a", "b", "eps", "m")

# Call fitOdeModel using the PORT algorithm.
fittedhp <- fitOdeModel(hp, whichpar = parms, obstime, yobs,
...
displayfittedhp <- hp
parms(displayfittedhp)[whichpar] <- coef(fittedhp)
