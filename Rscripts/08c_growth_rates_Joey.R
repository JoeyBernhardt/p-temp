...
algae.growth <- algae %>% 
	select(biovol, uniqueID) %>%
	as.data.frame() %>%
	group_by(uniqueID) %>% 
	mutate_each(funs(. / lag(.) - 1)) %>% 
	rename(growth.rate = biovol) %>%
	as.data.frame() %>%
	select(-uniqueID) %>% 
	bind_cols(algae, .) %>% 
	select(growth.rate, uniqueID, P, inverse_temp, treatment) %>% 
	dplyr::group_by(P, inverse_temp, treatment) %>% 
	filter(!is.na(growth.rate)) %>% 
	summarise_each(., funs(max, mean)) %>%
# 	rename(max_biovol_growth = max,
# 				 mean_biovol_growth = mean)
	# 		rename(max_daph_population_growth = max,
	# 					 mean_daph_population_growth = mean)
	# 		group_by(P) %>% 
	# 			do(tidy(lm(log(mean) ~ inverse_temp, data = .), conf.int = TRUE)) %>%
	...
	# 		ggplot(data =., aes(x = P, y = estimate)) + geom_point() + 
	# 		geom_errorbar(aes(ymin=conf.low, ymax = conf.high), width=.2)
	ggplot(data = ., aes(x = inverse_temp, y = log(growth.rate_mean), group = treatment, color = treatment)) + geom_point(aes(shape = P, size = 4)) + 
	geom_smooth(method = "lm") + theme_bw() + 
	theme(axis.text=element_text(size=16), axis.title=element_text(size=16,face="bold")) +
	ylab("grazed phytoplankton log max biovolume growth rate") + xlab("temperature, 1/kt")
ggsave("p-temp-figures_files/figure-html/max_grazed_phytoplankton_biovol_growth_arrhenius.png")
...
#### algae growth rates figures
algae.growth.rates %>% 
	ggplot(data = ., aes(x = inverse_temp, y = log(max_biovol_growth), group = Ptreatment, color = Ptreatment)) + geom_point(size = 8) + 
	geom_smooth(method = "lm", se = FALSE) + 
	theme_bw() + 
	theme(axis.text=element_text(size=16), axis.title=element_text(size=16,face="bold")) +
	scale_color_brewer(palette="Paired") + ylab("log max biovolume growth rate") + xlab("temperature, 1/kt") +
	scale_x_reverse() +
theme_bw() +
	theme(
		...
		panel.border = element_rect(color = "white"),
		plot.background = element_rect(fill = "transparent",colour = NA)) + 
	theme(legend.background = element_rect(color = NA), legend.margin = unit(1, "cm")) +
...
ggsave("test.png", bg = "transparent", width = 10, height = 8)
...
algae %>% 
	filter(date == "MAY4") %>% 
	group_by(uniqueID, inverse_temp, P, treatment) %>% 
# summarise(max.biovol = max(biovol)) %>%
	ggplot(data = ., aes(x = factor(P), y = log(biovol), fill = treatment)) + geom_boxplot() + facet_wrap( ~ inverse_temp)
ggsave("p-temp-figures_files/figure-html/final_time_biovol.png")
...
algae %>% 
	...
	group_by(uniqueID, inverse_temp, P, treatment, Ptreatment) %>% 
	summarise(max.biovol = max(biovol)) %>%
	ggplot(data = ., aes(x = inverse_temp, y = log(max.biovol), group = Ptreatment, color = Ptreatment)) + geom_point(aes(shape = P), size = 4) + 
	geom_smooth(method = "lm", se = FALSE) + 
	theme_bw() + 
	theme(axis.text=element_text(size=16), axis.title=element_text(size=16,face="bold")) +
	scale_color_brewer(palette="Paired") + ylab("log max biovolume") + xlab("temperature, 1/kt")
...
daphnia %>% 
		filter(date == "MAY4") %>% 
		group_by(P) %>% 
	ggplot(data =., aes(x = inverse_temp, y = log(biovol), group = P, color = P)) + geom_point(size = 6) +
				 	geom_smooth(method = "lm") +
	theme_bw() + 
	theme(axis.text=element_text(size=16), axis.title=element_text(size=16,face="bold")) +
	ylab("log algal biovolume, final") + xlab("temperature, 1/kt")


daphnia %>% 
	filter(date == "MAY4") %>% 
	group_by(P) %>% 
	ggplot(data =., aes(x = inverse_temp, y = log(dapnia_count), group = P, color = P)) + geom_point(size = 4) +
	geom_smooth(method = "lm") +
	theme_bw() + 
	theme(axis.text=element_text(size=16), axis.title=element_text(size=16,face="bold")) +
	ylab("log daphnia population abundance, final") + xlab("temperature, 1/kt")
...
algae %>% 
	...
	theme_bw() + 
	theme(axis.text=element_text(size=16), axis.title=element_text(size=16,face="bold")) +
	ylab("log ungrazed algal biovolume, final") + xlab("temperature, 1/kt")
