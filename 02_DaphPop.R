...
Daph <- left_join(Daph_raw, UniqueID_treat, by = "UniqueID")
...
Daph_tot$sample_date <- factor(Daph_tot$sample_date, levels = c("3/31/2016", "4/5/2016", "4/8/2016", "4/12/2016", "4/16/2016", "4/19/2016", "4/26/2016"))
...
Daph_tot %>% 
	...
	geom_point(aes(shape = factor(Daph_tot$treatment)))
...
Daph_tot %>% 
	filter(sample_date == "4/26/2016") %>% 
	filter(temperature.y != "20") %>% 
	# ggplot(aes(x = temperature.y, y = daph_tot, color = treatment.y)) + geom_point()
	lm(daph_tot ~ temperature.y, data = .) %>% 
	summary()

Daph_tot %>% 
	filter(temperature.y != "20") %>% 
	ggplot(data = ., aes(x = factor(temperature.y), y = daph_tot, color = treatment.y)) + geom_boxplot() + facet_wrap( ~ sample_date)
